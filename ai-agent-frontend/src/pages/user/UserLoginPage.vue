  ...
  <div class="auth-page" :class="themeClass">
    ...
    <section class="auth-hero" aria-label="AI 超级智能体应用平台">
      ...
      <div class="hero-copy">
        ...
        <h1 class="welcome-title" data-text="欢迎回来">欢迎回来</h1>
      </div>
    </section>
...
const getDefaultAppPath = (defaultAiApp?: string) => {
  const pathMap: Record<string, string> = {
    home: '/',
    'stock-master': '/stock-master',
    'super-agent': '/super-agent',
  }
  return pathMap[defaultAiApp || 'home'] || '/'
}

const getRedirectPath = (defaultAiApp?: string) => {
  const redirect = route.query.redirect
  if (typeof redirect !== 'string' || !redirect) {
    return getDefaultAppPath(defaultAiApp)
  }
...
const handleSubmit = async (values: any) => {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      router.push({
        path: getRedirectPath(preference.defaultAiApp),
        replace: true,
      })
    } else {
...
.welcome-title {
  ...
  width: fit-content;
  margin-top: 18px;
  font-family: 'STXingkai', '华文行楷', 'KaiTi', '楷体', 'Microsoft YaHei UI', serif;
  font-size: 6.2rem;
  font-weight: 700;
  letter-spacing: 0;
  background:
    linear-gradient(105deg, #e0f7ff 0%, #ffffff 30%, #7dd3fc 48%, #5eead4 64%, #f8fafc 100%);
  background-size: 240% 100%;
  -webkit-background-clip: text;
  background-clip: text;
  color: transparent;
  -webkit-text-stroke: 0.4px rgba(224, 242, 254, 0.18);
  filter:
    drop-shadow(0 0 10px rgba(103, 232, 249, 0.34))
    drop-shadow(0 16px 28px rgba(8, 145, 178, 0.14));
  animation: welcome-shine 6.4s ease-in-out infinite, welcome-float 5.2s ease-in-out infinite;
}
...
.welcome-title::before {
  ...
  -webkit-text-stroke: 7px rgba(34, 211, 238, 0.08);
  filter: blur(12px);
